...
data_tidy <- data %>%
  ...
  dplyr::select(
    Ro52_log10, Ro60_log10, Ro52_raw, Ro60_raw, ESSDAI_ana, nijisei
    ) %>%
  gather(
    var, val, -ESSDAI_ana, -nijisei
    )
...
mf.robustbase_smoothing <- function(D){
  ...
  gg_data <- ggplot(
    D,
    aes(x = as.numeric(ESSDAI_ana), y=val)
    )
  ...
  output <- list(
    plot(
      gg_data+gg_geom_1 + 
        ...
        gg_facet + summ_plot_theme
    ),
    res.GLM
    )
  names(output) <- sprintf('%s_%s.SS', unique(D$var), unique(D$nijisei))
  return(
   output
    )
  }
...
quartz(
  ...
  file = sprintf(
    '%s/%s_%s.pdf',
    dir.output, 
    'regressline.lmrob_ESSDAI_by_log10Ro_nijisei',
    .fn.data
    ),
  pointsize = 10,
  height = 7,
...
write_csv(
  ...
  path =
    sprintf(
      '%s/%s_%s_nonNijisei_%s.csv', 
      dir.output, 
      'result.regressline.lmrob_ESSDAI_by_','all',
      #        names(main.output)[i],
      .fn.data
    )
  )
