#' RscriptR --file=final_robust.reg_after_mice_190923.R  --vanilla --slave --args TRUE FALSE >  final_robust.reg_after_mice_190923.R.log
#' 
#' Copy of '/main_summarize_notNijisei_v05_190823'
#'   
#'    
...
dir.data           <- "../../Data"
fn.data            <- "AnalysisDataSet_v4.0_salvaged.RData"
...
print(suffix.df.coef.pooled.res)
...
data_SS <- data %>% 
  ...
  dplyr::filter(anti_SS_A_ab >= 34.9)
  
print(as.data.frame(data_SS))
...
# If you extract only patients with Primary-SS... ------------

print("Extraction of subjects with primary or secondary SS")

if(extract.primarySS==TRUE){
  long1 <-
    mice::complete(
      test.mice_SS, action='long', include=TRUE
      )
  long1 <- subset(long1, nijisei = "Primary")
  test.mice_SS <- as.mids(long1)}

if(extract.secondarySS==TRUE){
  long1 <-
    mice::complete(
      test.mice_SS, action='long', include=TRUE
      )
  long1 <- subset(long1, nijisei = "Secondary")
  test.mice_SS <- as.mids(long1)
  }

if(extract.secondarySS!=TRUE & extract.primarySS!=TRUE){
  long1 <-
    mice::complete(
      test.mice_SS, action='long', include=TRUE
      )
...
#sink("sink.list.res.lmrob.Ro60.mice.data_SS_nonNijisei.")

print("Robust linear regression (robustbase package)")
